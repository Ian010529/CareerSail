...
/**
 * 从香港招聘平台实时搜索岗位。仅返回内存结果，不执行 CSV 导入。
 * @param {Object} options
 * @param {string[]} options.sources - jobsdb | jijis | offertoday | hkstp | cyberport | indeed
 * @param {string} options.keyword - 搜索关键词
 * @param {number} options.maxPages - 每个平台最大页数
 * @param {string} options.graduationYear - 毕业年份
 * @param {string[]} options.keywords - 用户关键词
 * @param {boolean} options.excludeInternship - 是否排除实习
 * @returns {Promise<Array>} CareerSail 标准岗位列表
 */
async function searchPlatformJobs(options = {}) {
  const {
    sources = [],
    keyword = '',
    maxPages = 5,
  ...
  const supported = new Set(['jobsdb', 'jijis', 'offertoday', 'hkstp', 'cyberport', 'indeed']);
  const jobs = [];
  const seen = new Set();

  for (const source of sources) {
    if (!supported.has(source) || !TYPE_TO_CLASS[source]) {
      console.warn(`  ⚠ 未知香港岗位来源: ${source}`);
      continue;
    }

    console.log(`🔍 搜索 ${source} 平台岗位...`);
    try {
      const recruiter = createRecruiter(source);
      const sourceJobs = await recruiter.fetchJobs({ keyword, maxPages });
      for (const job of sourceJobs) {
        if (excludeInternship && isInternship(job)) continue;
        const cohort = verifyCohort(job, graduationYear);
        if (cohort.status === 'No') continue;

        const key = `${job.sourcePlatform || source}:${job.id || job.url || `${job.company}|${job.title}|${job.city}`}`;
        if (seen.has(key)) continue;
        seen.add(key);

        const freshness = checkFreshness(job);
        const matchDegree = computeMatchDegree(job, keywords);
        jobs.push({
          ...job,
          matchStatus: matchDegree,
          matchDegree,
      ...
      console.log(`  ✅ ${source} 找到 ${sourceJobs.length} 个岗位`);
    } catch (error) {
      console.error(`  ❌ ${source} 搜索失败: ${error.message || error}`);
    }
  }
